...
/**
 * This class is used to maintain a bookkeeping of CPU time usage.
 * Threads/Runnables that use this class should include the ThreadCpuTimer in
 * their class, and call the interface methods "startThreadCpuTimer" once the
 * computation begins and "endThreadCpuTimer" once the runnable stops.
 * 
 * The CPU time keeping is complicated by the fact that valid values can only be
 * obtained as long as a Thread is still running, hence a callback mechanism is
 * needed, see e.g.
 * http://stackoverflow.com/questions/17255498/implementing-callbacks-in-java-
 * with-runnable
 * 
 * The main thread (thread of the main method) is a special case. When
 * ThreadCpuTimeKeeper is created, it is assumed that this is done from the main
 * thread, and the start time for the main thread is immediately registered.
 * 
 * The main functionality comes from the method
 * "registerEndTimeMainThreadAndReportThreadTimeUsage" which first registers the
 * end time for the main thread and then produces a report of the time usage by
 * the different threads and the total time used.
 * 
...
public class ThreadCpuTimeKeeper implements ThreadCpuTimer {
  ...
  return df;
    ...
    private final Map<Long, ThreadCpuTimeStampDifference> threadIDToCpuTimingMap;
    private final WallClockTimeDifference wallClockTimeDifference;

    private ThreadCpuTimeKeeper(long mainThreadID,
      Map<Long, ThreadCpuTimeStampDifference> threadIDToCpuTimingMap,
  ...
  Map<Long, ThreadCpuTimeStampDifference> threadIDToCpuTimingMap = new HashMap<>();
  ThreadCpuTimeKeeper result = new ThreadCpuTimeKeeper(getMainTrheadID(),
    threadIDToCpuTimingMap, WallClockTimeDifference.createWallClockTimeDifference());
  ...
  if (threadIDToCpuTimingMap.containsKey(threadId)) {
      throw new RuntimeException(
        "Error: start time was already registerd for thread with ID: " + threadId);
  }
  ...
  threadIDToCpuTimingMap.put(threadId, threadCpuTimeStampDifference);
    }
  ...
  return 0;
    ...
    /**
     * This method registers the running start time for the provided thread. It
     * should be called inside the run method of the Runnable/Thread class for
     * which timing is desired, at the start of the run method.
     */
    @Override
  ...
  registerThreadStartTime(thread);
    ...
    /**
     * This method registers the running end time for the provided thread. It
     * should be called inside the run method of the Runnable/Thread class for
     * which timing is desired, at the end of the run method (just before it
     * returns)
     */
    @Override
  ...
  return printSummarySomeTimeUnit(totalCpuTimeNanoSeconds, totalUserTimeNanoSeconds,
    ...
    input -> convertNanoSecondsToSeconds(input), "seconds");
    }
  ...
  return result;
    ...
    private String summaryCSVNanoSeconds(long totalCpuTimeNanoSeconds,
      long totalUserTimeNanoSeconds, long totalSystemTimeNanoSeconds) {
  ...
  return result;
    ...
    private String headerCSV(
      java.util.function.Function<String, String> headerCSVTimeUnitFunction) {
  String result = "";
  ...
  return headerCSV(input -> headerCSVSeconds(input));
    ...
    /**
     * We use java 8's functional references to avoid code duplication, by
     * providing a reference to a summary generating function and a reference to
  ...
  result += summarySomeTimeUnitGenerationFunction.apply(
    getTotalCpuTimeSpentByAllExceptMainThread(),
  ...
  return summaryCSVSomeTimeUnit((arg1, arg2, arg3) -> summaryCSVSeconds(arg1, arg2, arg3),
    ...
    /**
     * First register the end time of the main thread, then
     * rgetTotalCpuTimeSpentByAllThreads(), getTotalUserTimeSpentByAllThreads(),
     * getTotalSystemTimeSpentByAllThreads()eport the time usage for all threads
     * and the total
     */
  ...
  result += printSummary(getTotalCpuTimeSpentByAllThreads(),
    getTotalUserTimeSpentByAllThreads(), getTotalSystemTimeSpentByAllThreads(),
    "all threads");

  result += NL + "\n-----------------------------------------------------";
  result += printSummary(getTotalCpuTimeSpentByMainThread(),
    getTotalUserTimeSpentByMainThread(), getTotalSystemTimeSpentByMainThread(),
    "main thread");
  ...
  private long getClockTimeSpent() {
     ...
     * 
     * User Time - This is the time the CPU spent running your code. It is
     * called user time because the CPU is used by an operation in a program
     * that a user has started.
     * 
     * System time - This is the time that the CPU was used for executing system
     * calls. It is literally the time the kernel is using the CPU for its
     * operations. You can think of I/O operations, context switches, inter
     * process communication, memory management, interrupt requests, etc.
     *
     *
     * systemTime is userTime + cpuTime, there is no way to get system time
     * directly
     * 
     * @author gemaille
     *
  ...
  /**
   * The start time method reads the cpu time first and the user time
   * last. We do this because we want to assure the passed cpu time will
   * be larger than the passed user time. Ideally we would like to read
   * both simultaneously, but that appears to be not possible.
   * 
   * @param thread
   * @return
   */
  ...
  /**
   * The end time method reads the user time first and the cpu time last.
   * We do this because we want to assure the passed cpu time will be
   * larger than the passed user time. Ideally we would like to read both
   * simultaneously, but that appears to be not possible.
   * 
   * @param thread
   * @return
   */
  ...
  private static ThreadCpuTimeStampDifference createThreadCpuTimeStampDifference(
    Thread thread) {
